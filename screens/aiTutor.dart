...
import 'dart:convert';
import 'package:agent36/screens/saved_conversations.dart';
...
class _AiTutorPageState extends State<AiTutorPage> {
  ...
  Future<void> _sendMessage() async {
    ...
    if (_useContext) {
      // Relate only to the last AI and last user message
      List<Map<String, String>> lastContext = [];
      // Find last AI/model message before the current user message
      for (int i = _messages.length - 1; i >= 0; i--) {
        if (_messages[i]['role'] == 'model' ||
            _messages[i]['role'] == 'model') {
          lastContext.add({'role': 'model', 'text': _messages[i]['text']!});
          break;
        }
      }
      // Add the current user message
      lastContext.add({'role': 'user', 'text': question});
      contextMessages = lastContext;
    } else {
